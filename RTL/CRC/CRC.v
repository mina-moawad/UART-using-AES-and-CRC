// CRC-16/XMODEM: x^16 + x^12 + x^5 + 1 POLYNOMIAL
module CRC #(
        ...
        if (!rst_n) begin
            ...
            if (start && !busy) begin
                ...
                if (counter < DATA_WIDTH) begin
                    if (crc_reg[CRC_WIDTH-1] ^ data_in_reg[DATA_WIDTH-1-counter]) begin
                        crc_reg <= {crc_reg[CRC_WIDTH-2:0], 1'b0} ^ POLYNOMIAL[CRC_WIDTH-1:0];
                    end
                    else begin
                    ...
                    data_with_crc <= {data_in_reg, crc_reg};
                    done          <= 1'b1;
                    busy          <= 1'b0;
                    counter       <= 8'd0;
                end
